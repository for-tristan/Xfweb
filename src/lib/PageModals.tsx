...
export function SearchModal({ open, onClose, query, setQuery, results, onSelect }: SearchModalProps) {
  ...
  if (!open) return null;
  return (
    <div ref={searchRef} role="dialog" aria-modal="true" aria-label="Search" className="search-modal-overlay" data-lenis-prevent style={{ position: 'fixed', inset: 0, background: 'color-mix(in srgb, var(--black) 97%, transparent)', zIndex: 9999, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 'max(60px, 14vh)' }} onClick={onClose}>
...
export function AuthModal({
  ...
  return (
    <div ref={authRef} role="dialog" aria-modal="true" aria-label={verificationStep === 'pending' ? 'Verify Email' : forgotStep === 'email' ? 'Forgot Password' : forgotStep === 'code' ? 'Verify Code' : tab === 'signin' ? 'Sign In' : 'Create Account'} className={`auth-modal-overlay${open ? ' active' : ''}`} data-lenis-prevent onClick={(e) => { if (e.target === e.currentTarget) { onClose(); setForgotStep('idle'); setVerificationStep('idle'); } }}>
...
export function ProfileModal({
  ...
  return (
    <div ref={profileRef} role="dialog" aria-modal="true" aria-label="Dashboard" className="dashboard-modal-overlay active" data-lenis-prevent onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
