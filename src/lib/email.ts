    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      ...
      try {
        await transport.sendMail({
          from: payload.from || `"X.Foundry" <${SMTP_USER}>`,
          to: payload.to,
  ...
  if (isResendConfigured()) {
    try {
      ...
      const resendPayload: any = {
        from: payload.from || `"X.Foundry" <${RESEND_FROM_EMAIL}>`,
        to: payload.to,
        subject: payload.subject,
        html: payload.html,
      };
  ...
  return `
    <div style="font-family: ${EMAIL_FONT}; max-width: 560px; margin: 0 auto; padding: 40px 24px; background: #ffffff;">
      <div style="margin-bottom: 36px;">
        <span style="font-size: 20px; font-weight: 700; color: ${COLOR_TEXT}; letter-spacing: -0.5px;">X<span style="color: ${COLOR_ACCENT};">.</span>Foundry</span>
      </div>
      ${content}
      <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid ${COLOR_BORDER};">
        <p style="color: #a3a3a3; font-size: 12px; line-height: 1.5; margin: 0;">X.Foundry<br>xfoundryy.vercel.app</p>
      </div>
    </div>
  `;
}
...
export async function sendEnrollmentStatusEmail(data: {
  ...
  return sendEmail({
    ...
    html: emailWrapper(`
      ...
      ${button(isApproved ? 'Go to Dashboard' : 'Visit X.Foundry', 'https://xfoundryy.vercel.app')}
    `),
  });
}
