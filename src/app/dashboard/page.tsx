...
function StatCard({ label, value, icon, color, sub, compact }: { label: string; value: string | number; icon: string; color: string; sub?: string; compact?: boolean }) {
  return (
    <div style={{
      background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: 2,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: compact ? 10 : 14 }}>
        <div style={{
          width: compact ? 28 : 36, height: compact ? 28 : 36, borderRadius: 2,
      ...
      <div style={{
        fontFamily: "'Orbitron', sans-serif", fontSize: compact ? 18 : 26, fontWeight: 900,
...
function ProgressBar({ pct, height = 6 }: { pct: number; height?: number }) {
  ...
  const color = clamped === 100 ? '#22c55e' : clamped >= 50 ? 'var(--primary-red)' : '#f59e0b';
  return (
...
function SectionHeader({ icon, title, color = 'var(--primary-red)' }: { icon: string; title: string; color?: string }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20,
    }}>
      <div style={{
        width: 4, height: 18, borderRadius: 1, background: color,
      }} />
      <h3 style={{
      ...
      {dayData.map((d, i) => {
        ...
        return (
          <div key={d.date} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div style={{ fontSize: 9, color: 'var(--text-dim)', fontWeight: 600, fontFamily: "'Space Grotesk', sans-serif" }}>
              {d.seconds > 0 ? formatDuration(d.seconds) : ''}
            </div>
            <div style={{
              width: '100%', height: h, borderRadius: 2,
              background: isToday
                ? 'linear-gradient(180deg, var(--primary-red), rgba(220,20,60,0.4))'
                : d.seconds > 0
                  ? 'rgba(220,20,60,0.2)'
                  : 'var(--input-bg)',
              border: isToday ? '1px solid rgba(220,20,60,0.4)' : '1px solid transparent',
              transition: 'height 0.5s ease',
              minWidth: 8,
            }} />
            <div style={{
              fontSize: 9, fontWeight: isToday ? 700 : 500,
              color: isToday ? 'var(--primary-red)' : 'var(--text-dim)',
              fontFamily: "'Space Grotesk', sans-serif",
...
export default function DashboardPage() {
  ...
  const statusBadge = (status?: string) => {
    ...
    const config: Record<string, { color: string; label: string }> = {
      ...
      approved: { color: '#22c55e', label: 'Active' },
  ...
  // ── Shared Navbar ──
  const renderNavbar = (activeLink?: string) => (
    <nav className={`navbar ${scrolled ? 'scrolled' : ''}`} id="navbar">
      ...
      <ul className={`nav-links ${mobileMenuOpen ? 'active' : ''}`} id="navLinks">
        ...
        {user && <li><Link href="/dashboard" className={activeLink === 'dashboard' ? 'active' : ''}>Dashboard</Link></li>}
  ...
  const renderModals = () => (
    <>
      ...
      <AuthModal open={authModalOpen} onClose={() => setAuthModalOpen(false)} tab={authTab} setTab={setAuthTab} message={authMessage}
        ...
        forgotStep={forgotStep} setForgotStep={setForgotStep} forgotEmail={forgotEmail} setForgotEmail={forgotEmail} forgotLoading={forgotLoading} onForgotSubmit={handleForgotSubmit}
  ...
  const Skeleton = ({ w, h }: { w: string; h: string }) => (
    <div style={{ width: w, height: h, borderRadius: 2, background: 'var(--input-bg)', animation: 'pulse 1.5s infinite' }} />
  );
  ...
  if (!user && !loading) {
    return (
      <>
        ...
        <AuthGate loading={loading} user={user} onSignIn={() => openAuthModal('signin', 'Sign in to access your dashboard')} onSignUp={() => openAuthModal('signup')} />
        {renderModals()}
        {renderNavbar()}
  ...
  return (
    <>
      <title>Dashboard | X.Foundry</title>
      <meta name="description" content="Your X.Foundry learning dashboard. Track courses, tests, and study progress." />
      ...
      <div className="page-transition-enter">
        ...
        <section className="course-hero">
          ...
          <div className="hero-content">
            ...
            <div className="hero-meta" style={{ marginTop: 24 }}>
              <div className="meta-pill"><i className="fas fa-chart-line" /> Progress</div>
              <div className="meta-pill"><i className="fas fa-crosshairs" /> Study</div>
            </div>
            <div className="breadcrumb" style={{ marginTop: 24 }}>
              <Link href="/" style={{ color: 'var(--text-light)' }}>Home</Link> <span>/</span> <span>Dashboard</span>
            </div>
          </div>
        </section>
        ...
        <section className="section" style={{ background: 'var(--black)' }}>
          <div className="container-max" style={{ paddingTop: 20, paddingBottom: 80, paddingLeft: isMobile ? 16 : undefined, paddingRight: isMobile ? 16 : undefined }}>
            ...
            {/* Welcome */}
            <div className="reveal" style={{ marginBottom: isMobile ? 24 : 36, display: 'flex', alignItems: 'center', gap: isMobile ? 12 : 16 }}>
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: 'var(--primary-red)', fontSize: 16, fontWeight: 900,
              ...
              <div>
                <h2 style={{
                  fontFamily: "'Orbitron', sans-serif", fontWeight: 900, fontSize: isMobile ? 15 : 18,
            ...
            <div style={{
              ...
              <div className="reveal" style={{
                ...
                borderRadius: 2, padding: isMobile ? '18px 14px' : '24px 22px',
              }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {dataLoading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '32px 16px' }}>
                      <i className="fas fa-book-open" style={{ fontSize: 28, color: 'var(--text-dim)', opacity: 0.2, display: 'block', marginBottom: 12 }} />
                      <p style={{ fontSize: 13, color: 'var(--text-dim)', margin: 0 }}>
                        No active courses yet.
                      </p>
                      <Link href="/#courses" style={{ fontSize: 12, color: 'var(--primary-red)', textDecoration: 'none', fontWeight: 700, marginTop: 8, display: 'inline-block' }}>
                        Browse Courses <i className="fas fa-arrow-right" style={{ fontSize: 10, marginLeft: 4 }} />
                      </Link>
                    </div>
                    ...
                    return (
                      <Link key={e.id} href={`/courses/${e.courseId}`} style={{
                        ...
                        background: 'var(--input-bg)', border: isComplete
                          ? '1px solid rgba(34,197,94,0.2)'
                          : '1px solid var(--border-color)',
                        borderRadius: 2, textDecoration: 'none', color: 'inherit',
                        transition: 'border-color 0.2s, background 0.2s',
                      }}
                        onMouseEnter={(ev) => { ev.currentTarget.style.borderColor = 'rgba(220,20,60,0.3)'; ev.currentTarget.style.background = 'rgba(220,20,60,0.03)'; }}
                        onMouseLeave={(ev) => { ev.currentTarget.style.borderColor = isComplete ? 'rgba(34,197,94,0.2)' : 'var(--border-color)'; ev.currentTarget.style.background = 'var(--input-bg)'; }}
                      >
                        <div style={{
                          width: 36, height: 36, borderRadius: 2,
                          background: isComplete ? 'rgba(34,197,94,0.1)' : 'rgba(220,20,60,0.08)',
                          border: isComplete ? '1px solid rgba(34,197,94,0.2)' : '1px solid rgba(220,20,60,0.15)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          flexShrink: 0,
                        }}>
                          <i className={`fas ${isComplete ? 'fa-check' : 'fa-book'}`}
                            style={{ fontSize: 13, color: isComplete ? '#22c55e' : 'var(--primary-red)' }}
                          />
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <span style={{
                              fontSize: 10, fontWeight: 700, color: pct === 100 ? '#22c55e' : 'var(--text-dim)',
                              fontFamily: "'Space Grotesk', sans-serif", minWidth: 32, textAlign: 'right',
                            }}>
                              {pct}%
                            </span>
                          </div>
                        ...
                        {isComplete && (
                          ...
                          >
                            <i className="fas fa-download" style={{ fontSize: 10, color: '#22c55e' }} />
                          </a>
                        )}
                      </Link>
                    );
              ...
              <div className="reveal" style={{
                ...
                <SectionHeader icon="fa-clipboard-check" title="TEST RESULTS" color="#22c55e" />
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {dataLoading ? (
                    <>
                    ...
                    <div style={{ textAlign: 'center', padding: '32px 16px' }}>
                      <i className="fas fa-clipboard-check" style={{ fontSize: 28, color: 'var(--text-dim)', opacity: 0.2, display: 'block', marginBottom: 12 }} />
                      <p style={{ fontSize: 13, color: 'var(--text-dim)', margin: 0 }}>
                        No test results yet.
                      </p>
                    </div>
                    ...
                    return (
                      <div key={t.id} style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                          ...
                          <span style={{
                            padding: '2px 8px', borderRadius: 2, fontSize: 8, fontWeight: 800,
            ...
            <div className="reveal" style={{
              ...
              <SectionHeader icon="fa-fire" title="STUDY ACTIVITY" color="#eab308" />
              ...
              {dataLoading ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 280px', gap: isMobile ? 20 : 28 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {[
                      { label: 'Today', value: formatDuration(studyStats?.todaySeconds || 0), icon: 'fa-sun', color: '#f59e0b' },
                      { label: 'This Week', value: formatDuration(studyStats?.weekSeconds || 0), icon: 'fa-calendar-week', color: '#dc143c' },
                      { label: 'Best Day', value: bestDay ? formatDuration(bestDay.duration) : '0s', sub: bestDay ? getDayLabel(bestDay.date) : undefined, icon: 'fa-trophy', color: '#22c55e' },
                      { label: 'All Time', value: formatDuration(studyStats?.allTimeSeconds || 0), icon: 'fa-infinity', color: '#a855f7' },
                    ].map((s, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 9, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: 0.8, fontWeight: 600 }}>
                            {s.label}
                          </div>
                          {s.sub && (
