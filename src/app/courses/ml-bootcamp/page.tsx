...
export default function MLBootcampPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/courses/my-enrollments')
      ...
      .then((data) => {
        if (data?.enrollments) {
          const match = data.enrollments.find((e: Enrollment) => e.courseId === 'ml-bootcamp');
  ...
  useEffect(() => {
    ...
    fetch('/api/courses/modules?courseId=ml-bootcamp')
      .then((res) => res.ok ? res.json() : null)
      .then((data) => {
        if (data?.modules) {
  ...
  useEffect(() => {
    if (!user) return;
    fetch('/api/courses/certificate?courseId=ml-bootcamp')
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleEnroll = (e: React.FormEvent) => {
    ...
    if (!formName || !formEmail || !formLevel) { toast({ title: 'Error', description: 'Please fill in all required fields', variant: 'destructive' }); return; }
    openConfirm(
      'Confirm Enrollment',
      'Are you sure you want to enroll in "ML Engineer Bootcamp"? An admin will review your request.',
      async () => {
        setEnrolling(true);
        try {
          const res = await fetch('/api/courses/enroll', {
            ...
            body: JSON.stringify({ courseId: 'ml-bootcamp', courseName: 'ML Engineer Bootcamp', courseLevel: 'Advanced', duration: '8 Weeks', experienceLevel: formLevel, motivation: formMotivation }),
          });
  ...
  const handleDownloadCert = async () => {
    ...
    try {
      const res = await fetch('/api/courses/certificate?courseId=ml-bootcamp&format=pdf');
      if (res.ok) {
  ...
  return (
    <>
      <title>ML Engineer Bootcamp | X.Foundry Courses</title>
      <meta name="description" content="Free 8-week Machine Learning Engineer Bootcamp by X.Foundry. Learn deep learning, NLP, computer vision, TensorFlow, PyTorch, and MLOps with hands-on projects." />
      <meta property="og:title" content="ML Engineer Bootcamp | X.Foundry Courses" />
      <meta property="og:description" content="Free 8-week intensive ML bootcamp covering deep learning, NLP, computer vision, and production deployment." />
      <meta property="og:type" content="website" />
      <meta name="twitter:card" content="summary_large_image" />
      <meta name="twitter:title" content="ML Engineer Bootcamp | X.Foundry Courses" />
      <meta name="twitter:description" content="Free 8-week intensive ML bootcamp covering deep learning, NLP, computer vision, and production deployment." />
      ...
      <nav className={`navbar ${scrolled ? 'scrolled' : ''}`} id="navbar">
        ...
        <ul className={`nav-links ${mobileMenuOpen ? 'active' : ''}`} id="navLinks">
          ...
          <li><a href="/#courses" className="active" onClick={(e) => { e.preventDefault(); scrollToSection('courses'); }}>Courses</a></li>
      ...
      <section className="course-hero">
        ...
        <div className="hero-content">
          <h1>Machine Learning<br /><span>Engineer Bootcamp</span></h1>
          <div className="hero-meta">
            <div className="meta-pill"><i className="fas fa-clock" /> 8 Weeks</div>
            <div className="meta-pill"><i className="fas fa-signal" /> Advanced Level</div>
            <div className="meta-pill"><i className="fas fa-certificate" /> Certificate Included</div>
          </div>
          <div className="breadcrumb" style={{ marginTop: 24 }}>
            <span style={{ color: 'var(--text-light)' }}>Home</span> <span>/</span> <span style={{ color: 'var(--text-light)' }}>Courses</span> <span>/</span> <span style={{ color: 'var(--text-light)' }}>ML Bootcamp</span>
          </div>
        </div>
      </section>
        ...
        <div className="container-max">
          <div className="course-layout">
            <div className="course-main reveal">
              ...
              <p>This comprehensive 8-week program is designed to take you from ML fundamentals to building production-ready AI systems. You&apos;ll work on real projects, learn industry best practices, and build a portfolio that showcases your skills to employers.</p>

              <h2 style={{ marginTop: 36 }}>What You&apos;ll Learn</h2>
              <div className="what-learn">
                <div className="learn-item"><i className="fas fa-check" /> Python for Data Science &amp; ML</div>
                <div className="learn-item"><i className="fas fa-check" /> Neural Networks &amp; Deep Learning</div>
                <div className="learn-item"><i className="fas fa-check" /> Natural Language Processing (NLP)</div>
                <div className="learn-item"><i className="fas fa-check" /> Computer Vision with OpenCV</div>
                <div className="learn-item"><i className="fas fa-check" /> TensorFlow &amp; PyTorch Frameworks</div>
                <div className="learn-item"><i className="fas fa-check" /> Model Deployment &amp; MLOps</div>
                <div className="learn-item"><i className="fas fa-check" /> Pandas, NumPy &amp; Data Manipulation</div>
                <div className="learn-item"><i className="fas fa-check" /> Real-world Project Portfolio</div>
              </div>
              ...
              <p>Basic programming knowledge (any language), familiarity with high school math, and a computer capable of running Python. No prior ML experience required — we start from the fundamentals.</p>
            </div>
              ...
              {enrollment ? (
                <div className="enroll-success show">
                  ...
                  <h4 style={{ textAlign: 'center', marginBottom: 8 }}>
                    {enrollment.status === 'approved' ? 'You\'re In!' : enrollment.status === 'declined' ? 'Request Declined' : 'Request Submitted!'}
                  </h4>
                  <p style={{ textAlign: 'center', marginBottom: 4 }}>
                    {enrollment.status === 'approved' ? 'Welcome to the ML Bootcamp! Check your email for next steps and Discord invite.' : enrollment.status === 'declined' ? 'Unfortunately your enrollment request was not approved at this time.' : 'Your enrollment request is being reviewed. We\'ll notify you once a decision is made.'}
                  </p>
                ...
                <div className="enroll-form">
                  ...
                  <p className="enroll-note">Limited spots available for this cohort</p>
                  ...
                  <form onSubmit={handleEnroll}>
                    ...
                    <div className="form-group">
                      ...
                      <select required value={formLevel} onChange={(e) => setFormLevel(e.target.value)}>
                    ...
                    <div className="form-group">
                      <label>Why do you want to learn ML?</label>
                      <input type="text" placeholder="Career change, hobby, etc." value={formMotivation} onChange={(e) => setFormMotivation(e.target.value)} />
                    </div>
                  ...
                  <div className="enroll-features">
                    ...
                    <div className="feature-check"><i className="fas fa-check" /> 8 weeks of structured content</div>
                    <div className="feature-check"><i className="fas fa-check" /> Hands-on projects</div>
                    <div className="feature-check"><i className="fas fa-check" /> Certificate of completion</div>
                    <div className="feature-check"><i className="fas fa-check" /> Discord community access</div>
      ...
      <section className="cta-section">
        <div className="cta-content reveal">
          ...
          <p>Reach out and we&apos;ll help you decide if this program is right for you.</p>
          <Link href="/#contact" onClick ={(e) => {
            ...
            }} className="btn btn-primary"><i className="fas fa-envelope" /> Contact Us</Link>
        </div>
      </section>
      </div>
