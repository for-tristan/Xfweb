  ...
  return (
    <div className={`reveal reveal-delay-${Math.min(index + 1, 5)}`} style={{ border: '1px solid var(--border-color)', borderRadius: 2, overflow: 'hidden' }}>
      <button onClick={() => setOpen(!open)} style={{
        ...
        <i className={`fa-solid fa-chevron-${open ? 'up' : 'down'}`} style={{ fontSize: 12, color: 'var(--accent)', flexShrink: 0 }} />
      </button>
      <div style={{ maxHeight: open ? 2000 : 0, overflow: open ? 'visible' : 'hidden', transition: 'max-height 0.4s ease' }}>
...
function EnrollButton({ courseId, courseName, onEnroll, status }: {
  ...
  if (status === 'approved') {
    return (
      <button className="btn btn-primary" disabled style={{ fontSize: 11, padding: '10px 20px', border: 'none', opacity: 0.6, cursor: 'default' }}>
  ...
  if (status === 'pending') {
    return (
      <button className="btn btn-secondary" disabled style={{ fontSize: 11, padding: '10px 20px', opacity: 0.5, cursor: 'default', borderColor: 'var(--warning-color)', color: 'var(--warning-color)' }}>
        <i className="fa-solid fa-hourglass-half" style={{ marginRight: 4 }} />Pending...
      </button>
    );
  }
  if (status === 'declined') {
    return (
      <button className="btn btn-primary" style={{ fontSize: 11, padding: '10px 20px', border: 'none' }} onClick={onEnroll}>
        Re-apply →
      </button>
    );
  }
...
useEffect(() => {
  ...
  const [theme, setTheme] = useState<string>(() => {
    ...
    const saved = localStorage.getItem('x-foundry-theme') || 'crimson';
    // Backwards compat: map old 'dark' to 'crimson'
    return saved === 'dark' ? 'crimson' : saved;
  });
  useEffect(() => {
    document.documentElement.setAttribute('data-theme', theme);
    document.documentElement.classList.toggle('dark', theme !== 'light' && theme !== 'sand');
  }, [theme]);
    ...
    try {
      ...
      if (res.ok) {
        if (data.requiresVerification) {
          ...
          setUser(data.user); setAuthModalOpen(false); setDqName(data.user.name); setDqEmail(data.user.email); setCqEmail(data.user.email); toast({ title: 'Account created!', description: `Welcome to X.Foundry, ${data.user.name}` });
        }
        ...
        <div className="services-grid">
          {servicesDataArr.length > 0 ? servicesDataArr.map((service, idx) => (
            <div key={service.id} className={`service-card reveal reveal-delay-${Math.min(idx + 1, 5)}`} style={{ cursor: 'pointer' }} onClick={() => router.push(`/services/${service.slug}`)}>
              ...
              <span className="service-link">Learn More <i className="fa-solid fa-arrow-right"></i></span>
            </div>
          )) : (
              ...
              <div key={course.id} className="course-card reveal">
                ...
                <div className="course-content">
                  ...
                  <div className="course-meta">
                    <span className="meta-item"><i className="fa-solid fa-clock"></i> {course.duration}</span>
                    <span className="meta-item"><i className="fa-solid fa-users"></i> {course.enrollmentCount != null ? `${course.enrollmentCount} enrolled` : 'Enrolling Now'}</span>
                  </div>
            ...
            <div key={m.id} className="team-card reveal" style={{ animationDelay: `${idx * 0.1}s` }}>
              ...
              {(m.linkedinUrl || m.githubUrl) && (
                <div className="team-socials">
                  {m.linkedinUrl && <a href={m.linkedinUrl} target="_blank" rel="noopener noreferrer" className="social-link" style={{ position: 'relative', zIndex: 11 }}><i className="fa-brands fa-linkedin-in"></i></a>}
                  {m.githubUrl && <a href={m.githubUrl} target="_blank" rel="noopener noreferrer" className="social-link" style={{ position: 'relative', zIndex: 11 }}><i className="fa-brands fa-github"></i></a>}
                </div>
              )}
            </div>
          )) : (
        ...
        <div className="contact-container">
          <div className="contact-info reveal">
            ...
            <ul className="info-list">
              ...
              <li className="info-item">
                <div className="info-icon"><i className="fa-solid fa-phone-alt"></i></div>
                <div className="info-text">
                  <h4>Call Us</h4>
                  <p>{contactPhone}</p>
                </div>
            ...
            <form onSubmit={handleEnrollSubmit}>
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
                  <i className="fa-solid fa-user" style={{ marginRight: 6, color: 'var(--accent)', fontSize: 11 }}></i>Full Name
                </label>
                <input
                  type="text"
                  value={user?.name || ''}
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
                  <i className="fa-solid fa-envelope" style={{ marginRight: 6, color: 'var(--accent)', fontSize: 11 }}></i>Email
                </label>
                <input
                  type="email"
                  value={user?.email || ''}
