...
function UnlockStudentPanel({ admin, modules, unlockLoading, handleUnlock, handleLock, handleUnlockAll, enrollments, fmt, actionLoading, courses }: {
  ...
  if (approvedEnrollments.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px', background: 'color-mix(in srgb, var(--card-bg) 60%, transparent)',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxHeight: 500, overflowY: 'auto' }}>
      {approvedEnrollments.map(enr => {
        ...
        return (
          <div key={enr.id} style={{ background: 'color-mix(in srgb, var(--card-bg) 60%, transparent)',
  backdropFilter: 'blur(20px) saturate(1.6)',
  WebkitBackdropFilter: 'blur(20px) saturate(1.6)', border: '0.5px solid color-mix(in srgb, var(--text-light) 10%, transparent)', padding: '16px 20px', borderRadius: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: enr.user.avatar ? 'transparent' : 'color-mix(in srgb, var(--accent) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--accent) 20%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--accent)', overflow: 'hidden', flexShrink: 0 }}>
                ...
                {userModules.map(mod => {
                  ...
                  return (
                    <button
                      ...
                      style={{ padding: '6px 14px', fontSize: 11, minWidth: 100, background: isUnlocked ? 'rgba(239,68,68,0.1)' : 'color-mix(in srgb, var(--accent) 10%, transparent)', border: isUnlocked ? '1px solid rgba(239,68,68,0.3)' : '1px solid color-mix(in srgb, var(--accent) 30%, transparent)', color: isUnlocked ? 'var(--error-color)' : 'var(--accent)' }}
                    >
