...
interface AdminAnalytics {
  ...
  recentActivity: Array<{ user: { name: string }; courseName: string; status: string; enrolledAt: string }>;
}
                ...
                {userModules.map(mod => {
                  ...
                  return (
                    <button
                      ...
                      style={{ padding: '6px 14px', fontSize: 11, minWidth: 100, background: isUnlocked ? 'rgba(239,68,68,0.1)' : 'rgba(220,20,60,0.1)', border: isUnlocked ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(220,20,60,0.3)', color: isUnlocked ? 'var(--error-color)' : 'var(--primary-red)' }}
                    >
...
export default function AdminPage() {
  ...
  const {
    user, loading, theme, toggleTheme, changeTheme, scrollToSection,
  ...
  const handleDeleteTeamMember = (id: string) => {
    ...
    }, { confirmLabel: 'Delete', danger: true, icon: 'fas fa-trash-alt' });
  };

  const [unlockLoading, setUnlockLoading] = useState<string | null>(null);
  const cursorDotRef = useRef<HTMLDivElement>(null);
  const cursorRingRef = useRef<HTMLDivElement>(null);
  useCustomCursor(cursorDotRef, cursorRingRef);
  ...
  const handleResetProgress = (p: ProgressEntry) => {
    openConfirm('Reset Progress', `Are you sure you want to reset ${p.user.name}'s progress for ${p.courseName}? This action cannot be undone.`, async () => {
      ...
      setProgressSaving(false);
    }, { confirmLabel: 'Reset', danger: true, icon: 'fas fa-undo' });
  };
  ...
  const handleSendCertificate = (p: ProgressEntry) => {
    ...
    openConfirm('Send Certificate', `Send certificate to ${p.user.name} (${p.user.email}) for ${p.courseName}?`, async () => {
      ...
      setCertSending(null);
    }, { confirmLabel: 'Send', danger: false, icon: 'fas fa-certificate' });
  };
  ...
  const handleLockTest = (testId: string, userId: string, userName: string) => {
    openConfirm('Lock Test', `Lock test for ${userName}? They will no longer be able to access it.`, async () => {
      ...
      setTestActionLoading(null);
    }, { confirmLabel: 'Lock', danger: true, icon: 'fas fa-lock' });
  };
  ...
  const handleRemove = (id: string) => {
    openConfirm('Remove Enrollment', 'Are you sure you want to permanently remove this enrollment? This action cannot be undone.', async () => {
      ...
      setActionLoading(null);
    }, { confirmLabel: 'Remove', danger: true, icon: 'fas fa-trash-alt' });
  };
  ...
  const handleRoleChange = (userId: string, newRole: string) => {
    ...
    openConfirm('Change Role', `Are you sure you want to ${action} this user?`, async () => {
      ...
      setActionLoading(null);
    }, { confirmLabel: 'Change Role', danger: false, icon: 'fas fa-user-shield' });
  };
  ...
  const handleDeleteUser = (userId: string) => {
    openConfirm('Delete User', 'Are you sure you want to permanently delete this user? This will also delete all their enrollments and quote requests. This action cannot be undone.', async () => {
      ...
      setActionLoading(null);
    }, { confirmLabel: 'Delete', danger: true, icon: 'fas fa-user-times' });
  };
                      ...
                      {u.id !== user!.id && (
                        <>
                          <button
                            ...
                            className="btn"
                            style={{ padding: '8px 16px', fontSize: 11, background: u.role === 'admin' ? 'transparent' : 'rgba(220,20,60,0.1)', border: u.role === 'admin' ? '1px solid rgba(234,179,8,0.4)' : '1px solid rgba(220,20,60,0.3)', color: u.role === 'admin' ? 'var(--warning-color)' : 'var(--primary-red)' }}
                          >
                          ...
                          return (
                            <div key={mod.id} style={{ background: 'var(--card-bg)', border: '1px solid var(--border-color)', padding: '20px 24px', borderRadius: 2 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                <i className="fas fa-user-check" style={{ fontSize: 10, color: unlockCount > 0 ? 'var(--success-color)' : 'var(--text-dim)' }}></i>
                  ...
                  {adminTeam.map(m => (
                    <div key={m.id} className="project-card" style={{ padding: '20px 28px', display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button onClick={() => handleDeleteTeamMember(m.id)} className="btn" style={{ padding: '6px 14px', fontSize: 11, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: 'var(--error-color)' }}><i className="fas fa-trash" style={{ marginRight: 4 }}></i>Delete</button>
                      </div>
                    </div>
                  ))}
                </div>
                                      ...
                                      {questionForm.options.map((opt, idx) => (
                                        <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                                          ...
                                          {questionForm.options.length > 2 && (
                                            <button onClick={() => removeQuestionOption(idx)} style={{ background: 'none', border: 'none', color: 'var(--error-color)', cursor: 'pointer', fontSize: 12, padding: '4px 8px' }}><i className="fas fa-times"></i></button>
                                          )}
                                        </div>
                                      ))}
                                      ...
                                      {students.map(({ user }) => {
                                        ...
                                        return (
                                          <div key={user.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', borderRadius: 2, gap: 12 }}>
                                            ...
                                            <span style={{ padding: '3px 10px', borderRadius: 2, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', background: isUnlocked ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)', color: isUnlocked ? 'var(--success-color)' : 'var(--error-color)', border: isUnlocked ? '1px solid rgba(34,197,94,0.25)' : '1px solid rgba(239,68,68,0.25)', flexShrink: 0 }}>
                                    ...
                                    {selectedTest.attempts.map((a) => {
                                      ...
                                      return (
                                        <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 16px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', borderRadius: 2, flexWrap: 'wrap' }}>
                                          ...
                                          <div style={{ fontSize: 20, fontWeight: 900, color: a.passed ? 'var(--success-color)' : 'var(--error-color)', fontFamily: "'Orbitron', sans-serif" }}>{pct}%</div>
                                          <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{a.score}/{a.totalPoints}</div>
                                          <span style={{ padding: '3px 10px', borderRadius: 2, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', background: a.passed ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)', color: a.passed ? 'var(--success-color)' : 'var(--error-color)', border: a.passed ? '1px solid rgba(34,197,94,0.25)' : '1px solid rgba(239,68,68,0.25)' }}>{a.passed ? 'PASSED' : 'FAILED'}</span>
                                          {a.submittedAt ? (() => {
                                            ...
                                            return (
                                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <span style={{ padding: '3px 10px', borderRadius: 2, fontSize: 11, fontWeight: 700, fontFamily: "'Orbitron', sans-serif", background: overTime ? 'rgba(239,68,68,0.12)' : 'rgba(34,197,94,0.08)', color: overTime ? 'var(--error-color)' : 'var(--success-color)', border: overTime ? '1px solid rgba(239,68,68,0.25)' : '1px solid rgba(34,197,94,0.15)' }}><i className="fas fa-stopwatch" style={{ marginRight: 5, fontSize: 9 }}></i>{durationStr}</span>
                                            ...
                                            <span style={{ padding: '3px 10px', borderRadius: 2, fontSize: 10, fontWeight: 700, background: 'rgba(234,179,8,0.1)', color: 'var(--warning-color)', border: '1px solid rgba(234,179,8,0.25)' }}>IN PROGRESS</span>
                                          )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(analytics.recentActivity || []).map((a, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: 2 }}>
                      ...
                      <span style={{ padding: '3px 12px', borderRadius: 2, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, background: a.status === 'approved' ? 'rgba(34,197,94,0.12)' : a.status === 'declined' ? 'rgba(239,68,68,0.12)' : 'rgba(234,179,8,0.12)', color: a.status === 'approved' ? 'var(--success-color)' : a.status === 'declined' ? 'var(--error-color)' : 'var(--warning-color)' }}>{a.status}</span>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                        {courseForm.features.map((feat, idx) => (
                          <span key={idx} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', background: 'var(--input-bg)', border: '1px solid var(--border-color)', borderRadius: 2, fontSize: 12, color: 'var(--text-light)' }}>
                            {feat}
                            <button onClick={() => removeCourseFeature(idx)} style={{ background: 'none', border: 'none', color: 'var(--error-color)', cursor: 'pointer', fontSize: 11, padding: 0 }}><i className="fas fa-times"></i></button>
                          </span>
                        ))}
                      </div>
                    )}
