...
function ThemeButton({ theme, currentTheme, onThemeChange, onClose }: {
  theme: ThemeInfo;
  currentTheme: string;
  onThemeChange: (id: string, e: React.MouseEvent) => void;
  onClose: () => void;
}) {
  const isActive = currentTheme === theme.id;
  return (
    <button
      onClick={(e) => { onThemeChange(theme.id, e); onClose(); }}
      style={{
        display: 'flex',
        alignItems: 'center',
        gap: 12,
...
export function ThemePicker({ currentTheme, onThemeChange }: ThemePickerProps) {
  ...
  const handleThemeChange = useCallback((newThemeId: string, e: React.MouseEvent) => {
    const x = e.clientX;
    const y = e.clientY;
